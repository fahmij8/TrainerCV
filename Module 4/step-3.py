        ...
        try:
            ...
            for score, (ymin, xmin, ymax, xmax), label in zip(pred_scores, pred_boxes, pred_labels):
                ...
                if (label == 'apple' or label == 'orange' or label == 'banana'):
                    ...
                    labelToShow = f"Freshness => {tags[result]} : {confidence}%"
                    text_size, _ = cv2.getTextSize(labelToShow, cv2.FONT_HERSHEY_SIMPLEX,  0.4, 2)
                    text_w, text_h = text_size
                    cv2.rectangle(img_boxes, (xmin+10, ymin-40), (xmin + text_w, ymin + text_h), (0, 0, 0), -1)
                    cv2.putText(img_boxes, labelToShow, (xmin+10, ymin-40), cv2.FONT_HERSHEY_SIMPLEX,  0.4, (255, 255, 255), 2 )
